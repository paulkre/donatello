...
{
    #region Mirevi Extension

    /// <summary>Takes a point array and fills it with the current marker positions</summary>
    /// <returns>The count of all current markers</returns>
    public int ReadMarkerPositions(Vector3[] points)
    {
        int count = 0;
        lock (m_frameDataUpdateLock)
        {
            foreach (var markerState in m_latestMarkerStates.Values)
            {
                points[count] = markerState.Position;
                count++;
            }
        }
        return count;
    ...
    /// <summary>Takes a list of points, clears it and fills it with the current marker positions</summary>
    public void ReadMarkerPositions(List<Vector3> points)
    {
        points.Clear();
        lock (m_frameDataUpdateLock)
            foreach (var markerState in m_latestMarkerStates.Values)
                points.Add(markerState.Position);
    }
    ...
    public long TimeStamp
    {
        get
        {
            return m_lastFrameDeliveryTimestamp.m_ticks;
        }
    }
